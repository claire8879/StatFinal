...
for (i in seq_along(targets)) {
  ...
  trainIndex <- sample(1:nrow(bankdf), floor(0.8*nrow(bankdf)))
  trainData <- bankdf[trainIndex, ]
  testData <- bankdf[-trainIndex, ]
  
  # Train the random forest model
  model <- randomForest(trainData[, !names(trainData) %in% i], trainData[, targets[i]], class = TRUE)
  
  # Make predictions using the test data
  predictions <- predict(model, testData)

 accuracy <- sum(predictions == testData[, targets[i]]) / length(predictions)
  cat("Accuracy for", targets[i], ":", round(accuracy, 2), "\n")
}

```

```{r}
tree <- getTree(model, k = 1)
rpart.plot(tree)
```
